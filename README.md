...
当输出格式为 `web` 且已经配置好 Worker 后，用户可以在网页发起绑定，然后在目标群里发送：

```text
/绑定博客 <一次性绑定码>
```

插件会把当前群号、当前发送者 ID 和绑定码回调给 Worker，Worker 验证通过后，用户即可回网页设置密码并完成登录。

相关配置位于 `web_blog` 分组：

- `worker_base_url`
- `worker_ingest_path`
- `worker_bind_verify_path`
- `worker_upload_token`
- `worker_bind_callback_token`
