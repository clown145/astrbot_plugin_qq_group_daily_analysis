...
| Web链接格式 | 需要先部署仓库内置的 Cloudflare Worker，并填写 `web_report` 配置组。 | 默认关闭 |
...
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/SXP-Simon/astrbot_plugin_qq_group_daily_analysis)

部署完成后，在 AstrBot 插件配置 WebUI 中修改：
...
- `web` 是新的第三种发送方式，默认关闭。
- 插件会继续使用当前模板体系渲染最终 HTML，再上传给 Worker。
- 所以 Worker 不需要模板文件，`worker/` 里只保留服务端源码。
