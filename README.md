...
### 📊 可视化报告
- **多种格式**: 支持图片、文本、PDF、网页链接输出格式
    - **精美图片**: 生成美观的可视化报告
    - **PDF报告**: 生成专业的PDF格式分析报告（需配置）
    - **网页链接**: 将当前模板渲染后的 HTML 上传到 Cloudflare Worker，再向群内发送链接
...
| Web链接格式 | 需要先部署 `worker/` 子目录对应的 Cloudflare Worker，并填写 `web_report` 配置组。 | 默认关闭 |
| 自定义LLM服务 | 用户可自行选取个人提供的服务商。 | 留空则回退到默认服务商 |

## 网页链接模式

如果你不想在群里直接发图片，可以把报告作为 HTML 上传到 Cloudflare Worker，然后在群里发送短链接。
...
这个按钮指向仓库里的 `worker/` 子目录。
该子目录是独立 Worker 项目，配置文件在 `worker/wrangler.jsonc`，不是 `wrangler.toml`。

部署完成后，在 AstrBot 的插件配置 WebUI 中修改：

- `基础设置 -> 输出格式` 设为 `web`
- `网页报告设置 -> 启用网页报告发送` 打开
- `网页报告设置 -> Worker API 地址` 填 Worker 域名，例如 `https://xxx.workers.dev`
- `网页报告设置 -> Worker 上传令牌` 填和 Worker 端 `UPLOAD_TOKEN` 一致的值
- `网页报告设置 -> 公开链接基础地址` 如果绑定了自定义域名就在这里填，没有可以留空
- `网页报告设置 -> 报告保留天数`、`上传超时时间（秒）` 按需调整

说明：

- `web` 是新的第三种发送方式，默认关闭。
- 插件会继续使用当前模板体系渲染 HTML，所以现有模板无需额外复制到 `worker/`。
- Worker 目录是独立子项目，适合直接用官方 Deploy Button 部署。
- Cloudflare 官方当前说明里，Deploy Button 支持子目录，但该子目录必须完全自包含；`worker/` 就是按这个要求整理的。
- 首次部署后，Cloudflare 会把该子目录项目克隆到部署者自己的 GitHub/GitLab 账号。后续更新请同步 fork 并重新部署，不建议反复重新点按钮。
