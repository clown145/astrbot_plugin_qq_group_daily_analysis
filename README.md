...
> [!TIP]
> 如果生成图片失败，日志显示 `Endpoint https://t2i.soulter.top/text2img failed`、`Endpoint https://t2i.rcfortress.site/text2img failed` 这种情况并回退到文本总结时，可考虑自部署 T2I 服务。
>
> <details>
...
>   - **说明**: 
>     1. **复制空间**：可访问上方“服务空间”地址并点击 **Duplicate Space** 复制到自己的账号下使用。
>     2. **配置填写**：在 **AstrBot 系统配置** 中填入相应的 **API 接口地址**（注意不是空间页面地址）。例如，若你的用户名为 `myuser`，则地址通常为 `https://myuser-astrbot-t2i-service.hf.space`。
...
> </details>
>
> **自部署 T2I 文档**：[https://docs.astrbot.app/others/self-host-t2i.html](https://docs.astrbot.app/others/self-host-t2i.html)
